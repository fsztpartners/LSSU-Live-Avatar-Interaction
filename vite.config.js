...
function heyGenTokenPlugin(env) {
  ...
  const handler = async (req, res) => {
    ...
    if (!apiKey) {
      res.statusCode = 500
      res.end(JSON.stringify({ error: 'missing_HEYGEN_API_KEY', hint: 'Add HEYGEN_API_KEY to .env (see .env.example).' }))
      return
    }
    ...
    let clientPayload = {}
    try { clientPayload = body ? JSON.parse(body) : {} } catch { /* empty */ }

    const avatarId = clientPayload.avatar_id || env.HEYGEN_DEFAULT_AVATAR_ID
    if (!avatarId) {
      res.statusCode = 400
      res.end(JSON.stringify({ error: 'missing_avatar_id', hint: 'Pass avatar_id or set HEYGEN_DEFAULT_AVATAR_ID in .env.' }))
      return
    }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), heyGenTokenPlugin(env)],
  }
})
